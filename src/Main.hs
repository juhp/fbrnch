...
main = do
  ...
  simpleCmdArgs (Just version) "Fedora branch building tool"
    ...
    , Subcommand "compare-tarballs" "Diff source tarballs" $
      compareTarballsCmd
      <$> optional (optionLongWith auto "max-line-length" "LENGTH" "Skip diff files longer than this [default 200]")
...
--      pure testBZlogin
    ...
    , Subcommand "copr" "Build package(s) in Fedora Copr" $
      ...
      <*> (flagWith' ListChroots 'l' "list-chroots" "Show project chroots" <|>
           ...
           flagLongWith' CoprFailures "failures" "Show build failures with build.log tails" <*> optional (strOptionLongWith "filter" "NEEDLE" "Filter to results matching NEEDLE") <|>
           -- FIXME: NonEmpty String
           pure CoprBuild)
